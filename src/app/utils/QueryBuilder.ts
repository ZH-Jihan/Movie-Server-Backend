class PrismaQueryBuilder<T, Where, Select> {
  ...
  search(fields: (keyof T)[]): this {
    ...
    if (this.options.where) {
      ...
      // @ts-ignore
      this.options.where = { OR: orConditions } as Where;
    }
    return this;
  }
  ...
  filter() {
    ...
    if (Object.keys(filters).length === 0) return this;

    // Merge with existing where: use AND to combine
    if (this.options.where) {
      // @ts-ignore
      this.options.where = { AND: [this.options.where, filters] } as Where;
    } else {
      // @ts-ignore
      this.options.where = filters as Where;
    }
    return this;
  }
  ...
  fields(): this {
    ...
    const fields = fieldsParam.split(",").map((f) => f.trim()) as (keyof T)[];
    const selectObj: Record<string, boolean> = {};
    for (const f of fields) {
      selectObj[f as string] = true;
    }
    // @ts-ignore
    this.options.select = selectObj as Select;
    return this;
  }
